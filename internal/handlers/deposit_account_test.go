...
func TestDepositAccountHandler(t *testing.T) {
	...
	testCases := []struct {
	...
	}{
		{
			...
			accountServBehavior: func(accServ *mock_services.MockAccount, accId int32, value decimal.Decimal) {
				accServ.EXPECT().Deposit(accId, value).Return(nil)
			},
		...
		},
		{
			...
			inputBody: `{"accountId":1,"value":"100.01"}`,
			accountServBehavior: func(accServ *mock_services.MockAccount, accId int32, value decimal.Decimal) {
				accServ.EXPECT().Deposit(accId, value).Return(errors.New("bla-bla-bla"))
			},
			expextedStatusCode:   400,
	...
	for _, testCase := range testCases {
		t.Run(testCase.name, func(t *testing.T) {
			...
			req, err := http.NewRequest("POST", "/balance/deposit",
